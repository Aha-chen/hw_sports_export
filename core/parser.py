...
class HuaweiParseError(RuntimeError):
    code = "PARSE_FAILED"


class IncorrectPasswordError(HuaweiParseError):
    code = "INCORRECT_PASSWORD"


class InvalidArchiveError(HuaweiParseError):
    code = "INVALID_ARCHIVE"


class UnsupportedExportSchemaError(HuaweiParseError):
    code = "UNSUPPORTED_EXPORT_SCHEMA"


class _UnsupportedDataSchema(ValueError):
    """标识 ZIP 内部数据结构不是当前支持的华为运动数组。"""


# process_single_activity 非成功返回值：与结果 dict 区分，供上层分类统计。
ACTIVITY_SKIP_DATE_FILTERED = "date_filtered"
ACTIVITY_SKIP_NO_EXPORTABLE_TRACK = "no_exportable_track"
...
class ParseOutcome:
    ...
    date_filtered_activities: int = 0
    no_exportable_activities: int = 0


def _is_password_error(exc):
    message = str(exc).lower()
    return "password" in message or "bad pwd" in message
...
def process_single_activity(activity, start_ts, end_ts, output_dir):
    ...
    返回值：
    - dict：成功生成 TCX 的活动摘要；
    - ACTIVITY_SKIP_DATE_FILTERED：被日期范围排除；
    - ACTIVITY_SKIP_NO_EXPORTABLE_TRACK：无 GPS/配速等可导出轨迹。
    """
    start_time_ms = activity.get("startTime", 0)
    if start_ts and start_time_ms < start_ts:
        return ACTIVITY_SKIP_DATE_FILTERED
    if end_ts and start_time_ms > end_ts:
        return ACTIVITY_SKIP_DATE_FILTERED
    ...
    if "HW_EXT_TRACK_SIMPLIFY@is" in attr:
        try:
            ...
            if not summary_cadence and "mExtendTrackDataMap" in summary_data:
                ...
                if cad_str:
                    summary_cadence = int(cad_str)
        except (TypeError, ValueError):
            # 部分旧设备会写入空值或非数字摘要；保留默认步频继续解析轨迹。
            pass
            ...
            if tp == "lbs":
                ...
                for p in p_parts:
                    ...
                    if k == "k":
                        ...
                        v = float(val)
                if t_ms is not None and v is not None:
                    hr_data[int(t_ms / 1000)] = v
            elif tp == "s-r":
                t_ms, v = None, None
                for p in p_parts:
                    if "=" not in p:
                    ...
                    if k == "k":
                        ...
                        v = float(val)
                if t_ms and v is not None:
                    cad_data[int(t_ms / 1000)] = v
            elif tp == "alti":
                t_ms, v = None, None
                for p in p_parts:
                    if "=" not in p:
                ...
                for p in p_parts:
                    ...
                    if k in ("sn", "st", "sd", "sp", "shr", "sc"):
                        try:
                            ...
                            # 单个手动分段字段异常时忽略该字段，后续完整性判断决定是否采用。
                            pass
                if "sn" in sec:
    ...
    valid_gps_data = [
        pt
        for pt in gps_data
        if "lat" in pt and "lon" in pt and (abs(pt["lat"]) > 0.1 or abs(pt["lon"]) > 0.1)
    ]
    ...
    for pt in gps_data:
        if previous_valid_pt is not None:
            ...
            dist = haversine_distance(
                prev["wgs_lat"], prev["wgs_lon"], pt["wgs_lat"], pt["wgs_lon"]
            )
            if dist < 1000:
                cum_dist += dist
            else:
        ...
        if alti is not None:
            last_alti = alti
        pt["alti"] = (
            alti if alti is not None else (pt["alt"] if pt["alt"] is not None else last_alti)
        )
    ...
    if len(final_trackpoints) > 1 and watch_total_time_s > 0:
        ...
        if source_duration > 0:
            ...
            for point in final_trackpoints:
                ...
                normalized_ts = (
                    activity_start
                    + (point_ts - source_start) * watch_total_time_s / source_duration
                )
    ...
    if points_added == 0 and (sport_type == 101 or sport_type == 256):
        ...
        if pm_data and watch_total_dist > 0:
            ...
            if final_trackpoints:
                ...
                if (
                    datetime.fromisoformat(
                        final_trackpoints[-1]["time"].replace("Z", "+00:00")
                    ).timestamp()
                    < final_time
                ):
                    final_trackpoints.append(
            ...
            if len(indoor_times) > 2:
                ...
                valid_times = [
                    t for t in sorted_times if start_time_s <= t <= start_time_s + total_time_s + 60
                ]
                ...
                if len(valid_times) > 2:
                    ...
                    for t in valid_times:
                        progress = (
                            (t - valid_times[0]) / actual_duration if actual_duration > 0 else 0
                        )
            ...
            if points_added == 0:
                ...
                final_trackpoints.append(
                    {"time": start_time_iso, "dist": 0.0, "hr": summary_hr, "cad": summary_cadence}
                )
                final_trackpoints.append(
                    {
                        "time": end_time_iso,
    ...
    if points_added > 0:
        ...
        if section_data and len(final_trackpoints) > 2:
            ...
            for pt in final_trackpoints:
                ...
                if (
                    boundary_idx < len(sec_boundaries)
                    and pt["dist"] >= sec_boundaries[boundary_idx]
                ):
        ...
        for lap_pts in laps:
            ...
            lap_cal = (
                int(total_calories * (lap_dist / watch_total_dist)) if watch_total_dist > 0 else 0
            )
            ...
            ET.SubElement(lap_node, f"{{{NS}}}TriggerMethod").text = (
                "Manual" if section_data else "Distance"
            )
            ...
            for pt in lap_pts:
                ...
                if pt.get("cad") is not None:
                    ...
                    cad_val = (
                        int(pt["cad"] / 2) if sport_type in (4, 5, 101, 256) else int(pt["cad"])
                    )
        ...
        if len(final_trackpoints) > 0:
            ...
            for i in range(0, len(final_trackpoints), chart_step):
                ...
                # 心率数据点
                if pt.get("hr") is not None:
                    hr_series.append({"time": time_str, "elapsed": elapsed_s, "hr": int(pt["hr"])})
    ...
    # 无 GPS 且无可用配速节点时无法生成可导入轨迹。
    return ACTIVITY_SKIP_NO_EXPORTABLE_TRACK


def parse_huawei_zip(
    zip_path, password, output_dir, start_date=None, end_date=None, progress_callback=None
):
    """
    ...
    date_filtered_activities = 0
    no_exportable_activities = 0

    def collect_completed(pending_futures, total_activities, completed_activities):
        nonlocal skipped_activities, date_filtered_activities, no_exportable_activities
        for future in concurrent.futures.as_completed(pending_futures):
            ...
            if isinstance(res, dict):
                ...
                if progress_callback:
                    progress_callback(completed_activities, total_activities, res.get("sport", ""))
            else:
                # 日期过滤与无轨迹跳过不算解析失败；异常路径已记入 issues。
                if res == ACTIVITY_SKIP_DATE_FILTERED:
                    date_filtered_activities += 1
                    skipped_activities += 1
                elif res == ACTIVITY_SKIP_NO_EXPORTABLE_TRACK:
                    no_exportable_activities += 1
                    skipped_activities += 1
                if progress_callback and completed_activities <= total_activities:
                    progress_callback(completed_activities, total_activities, "")
        return completed_activities

    try:
        with pyzipper.AESZipFile(
            zip_path, "r", compression=pyzipper.ZIP_LZMA, encryption=pyzipper.WZ_AES
        ) as z:
            # 第一阶段：快速扫描获取 JSON 文件列表（不读取内容）
            json_files = []
            for name in z.namelist():
                if (
                    name.startswith("Motion path detail data & description/motion path detail data")
                    and name.endswith(".json")
                    and "Description" not in name
                ):
            ...
            if total_files == 0:
                raise UnsupportedExportSchemaError(
                    "Archive does not contain a supported Huawei activity data file"
                )
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for json_file in json_files:
                    ...
                    try:
                        with z.open(json_file, pwd=password_bytes) as f:
                            ...
                            if not isinstance(activities, list):
                                raise _UnsupportedDataSchema("Activity JSON must contain an array")

                            # 更新总数并立即提交处理
                            total_activities += len(activities)
                            for activity_index, act in enumerate(activities):
                                future = executor.submit(
                                    process_single_activity, act, start_ts, end_ts, output_dir
                                )
                                pending_futures[future] = f"{json_file}#{activity_index}"

                    except RuntimeError as exc:
                        if _is_password_error(exc):
                            raise IncorrectPasswordError(
                                "Incorrect extraction password provided"
                            ) from exc
                        issues.append(
                            ParseIssue(
                                ...
                                scope="file",
                                source=json_file,
                                message="Activity data file could not be read",
                            )
                        )
                        continue
                    except pyzipper.zipfile.BadZipFile as exc:
                        raise InvalidArchiveError("Invalid ZIP file or corrupted archive") from exc
                    except _UnsupportedDataSchema:
                        issues.append(
                            ParseIssue(
                                code="UNSUPPORTED_EXPORT_SCHEMA",
                                scope="file",
                                source=json_file,
                                message="Activity data schema is not supported",
                            )
                        )
                        continue
                        ...
                        issues.append(
                            ParseIssue(
                                ...
                                scope="file",
                                source=json_file,
                                message="Activity data file is invalid or unsupported",
                            )
                        )
                        continue
                    except Exception:
                    ...
                    if processed_files % 5 == 0:
                        completed_activities = collect_completed(
                            pending_futures,
                            total_activities,
                            completed_activities,
                        )
    ...
    except (IncorrectPasswordError, InvalidArchiveError, UnsupportedExportSchemaError):
        raise
    except pyzipper.zipfile.BadZipFile as exc:
        raise InvalidArchiveError("Invalid ZIP file or corrupted archive") from exc
    except RuntimeError as e:
        if _is_password_error(e):
            raise IncorrectPasswordError("Incorrect extraction password provided") from e
        raise HuaweiParseError("Unable to parse Huawei archive") from e
    ...
    return ParseOutcome(
        ...
        date_filtered_activities=date_filtered_activities,
        no_exportable_activities=no_exportable_activities,
    )
